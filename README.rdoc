...
Copyright:: Copyright (c) 2013 Future Ad Labs
...
PlayCaptcha for Rails > 3.0, add this to your Gemfile:
...
=== PlayCaptcha.configure

You may use the block style configuration. The following code could be placed
into a +config/initializers/playcaptcha.rb+ when used in a Rails project.
...
== PlayCaptcha#with_configuration
...
<tt>:ssl</tt>::         Uses secure http for reCAPTCHA widget (default +false+, but can be changed by setting +config.use_ssl_by_default+)
...
<tt>:ajax</tt>::        Render the dynamic AJAX reCAPTCHA per the API. (default +false+)
<tt>:public_key</tt>::  Your reCAPTCHA public API key, takes precedence over the ENV variable (default +nil+)
